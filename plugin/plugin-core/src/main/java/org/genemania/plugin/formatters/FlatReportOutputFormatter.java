...
public class FlatReportOutputFormatter implements IOutputFormatter {
	...
	public FlatReportOutputFormatter(DataSet data, IGeneProvider provider) {
		fData = data;
		fProvider = provider;
	}
	
	public void format(OutputStream out, ViewState viewState) {
		ReportExporter exporter = new TextReportExporter(fProvider);
		ManiaReport report = new ManiaReport(viewState, fData);
		exporter.export(report, out);
	}

	public String getExtension() {
		return "report.txt"; //$NON-NLS-1$
	}
