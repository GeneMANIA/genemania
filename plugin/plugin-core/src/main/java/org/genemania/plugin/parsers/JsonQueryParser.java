...
public class JsonQueryParser extends AbstractQueryParser {
	public Query parse(DataSet data, Reader reader, IQueryErrorHandler handler) throws IOException {
		...
		try {
			Organism organism = parseOrganism(data, root.get("organism").getTextValue()); //$NON-NLS-1$
	...
	private Collection<Group<?, ?>> parseNetworks(DataSet data, JsonNode root, Organism organism) {
		List<InteractionNetworkGroup> allGroups = data.getMediatorProvider().getNetworkMediator().getNetworkGroupsByOrganism(organism.getId());
		
		List<Group<?, ?>> groups = new ArrayList<Group<?, ?>>();
		for (String groupName : new OneUseIterable<String>(root.getFieldNames())) {
			...
			for (JsonNode node : new OneUseIterable<JsonNode>(root.get(groupName).getElements())) {
