 ...
 *
 * just linke MemOrganismMediator, but taking long's for network ids.
 * transitory stage between data changes ...
 */
public class MemOrganismMediator2 implements OrganismMediator {

    long organismId;
    int groupSize;
    long[] networkIds;

    public MemOrganismMediator2(long organismId, long[] networkIds, int groupSize) {
    ...
    InteractionNetworkGroup makeGroup(long organismId, int groupId, Collection<InteractionNetwork> networks) {
        InteractionNetworkGroup group = new InteractionNetworkGroup();
        group.setId(groupId);
